...
try:
    ...
    if conexion: # Verificar si la conexión está abierta
        ...
        #! sin_path_grupo = func_path_grupo_vacio(instrumentos_mt5, instrumentos_path) #Borrar
        insert, update, no_update = func_actualiza_path_instrumentos(instrumentos_mt5,instrumentos_path)
        # print(f"\n\nNuevo ({len(insert)}): {insert}\n\nActualizado ({len(update)}): {update}\n\nNo Actualizado ({len(no_update)}): {no_update}")
        # func_ins_instrumento_path(conexion, insert)
        # func_upd_path_instrumento(conexion, update)
